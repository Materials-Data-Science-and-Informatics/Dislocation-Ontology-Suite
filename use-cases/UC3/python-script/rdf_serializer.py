...
#dislocation-sim-model-ontology 
...
def rdf_serializer(cif_data, space_group_data, node_data, linker_data, loop_data, iri):
    ...
    for i, linker in enumerate(linker_data):
        ...
        g.add((segment, SIM.hasStartNode, example['node_{}'.format(start_node_id)]))
        g.add((segment, SIM.hasEndNode, example['node_{}'.format(end_node_id)]))
        g.add((segment, SIM.isSegmentOf, example['dislocation_{}_discretized_line'.format(dislocation_id)]))
