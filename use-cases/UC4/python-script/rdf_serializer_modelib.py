...
def crystal_rdf_serializer(cif_data, space_group_data, mat_info, ns):
    ...
    g.parse("../../SIM/dislocation-sim-model-ontology.owl", format="xml")
...
def dislocation_structure_serializer(mat_info, cif_data, init_micro, node_data, linker_data, loop_data, ns, key, edge, is_relaxed):
    ...
    g.add((cube_shape, RDF.type, SIM.Cube))
    g.add((cube_edge_length, RDF.type, SIM.Length))
    g.add((cube_edge_length_qv, RDF.type, QUDT.QuantityValue))
    g.add((cube_shape, SIM.hasLength, cube_edge_length))
    ...
    if key=='input':
        ...
        g.add((init_dislocation_density, RDF.type, SIM.DislocationDensity))
    ...
    g.add((dislocation_structure, RDF.type, SIM.DislocationStructure))
    if key=='input': 
        g.add((ddd_sim, SIM.hasInputDislocationStructure, dislocation_structure))
        g.add((dislocation_structure, SIM.isRelaxed, Literal(False,  datatype=XSD.boolean)))
        g.add((dislocation_structure, SIM.hasShape, cube_shape))
        ...
        g.add((ddd_sim, SIM.hasOutputDislocationStructure, dislocation_structure))
        g.add((dislocation_structure, SIM.isRelaxed, Literal(is_relaxed,  datatype=XSD.boolean)))
        g.add((dislocation_structure, SIM.hasShape, cube_shape))
    ...
    for node in node_data:
        ...
        g.add((node_individual, RDF.type, SIM.Node))
    ...
    for loop in loop_data:
        ...
        g.add((line, SIM.hasNumericalRepresentation, discretized_line))
        g.add((discretized_line, RDF.type, SIM.DiscretizedLine))
        g.add((discretized_line, SIM.slipArea, Literal(slip_area, datatype=XSD.double)))
    ...
    for i, linker in enumerate(linker_data):
        ...
        g.add((segment, RDF.type, SIM.Segment))
        g.add((segment, SIM.hasStartNode, ns['node_{}_{}'.format(start_node_id, key)]))
        g.add((segment, SIM.hasEndNode, ns['node_{}_{}'.format(end_node_id, key)]))
        g.add((ns['dislocation_{}_discretized_line_{}'.format(dislocation_id, key)], SIM.hasSegment, segment))
