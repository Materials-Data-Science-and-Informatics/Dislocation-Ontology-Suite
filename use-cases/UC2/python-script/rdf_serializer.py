...
# function to serializing the CIF and space group data into the Resource Description Framework (RDF) using the CSO
# returning graph object of rdflib
def rdf_serializer(cif_dict, space_group, slip_plane_configs, slip_plane_normals, uri, mat_id):
    #Material data URI
    ...
    space_group_data = space_group['spacegroup']
    ...
    ## length data value
    length_a = Literal(cif_dict['_cell_length_a']*1e-10, datatype=XSD.double) 
    length_b = Literal(cif_dict['_cell_length_b']*1e-10, datatype=XSD.double)
    length_c = Literal(cif_dict['_cell_length_c']*1e-10, datatype=XSD.double)
    ...
    for i in range(len(cif_dict['_atom_site_occupancy'])):
        ...
        for (i, (key, list_item)) in enumerate(slip_plane_normals.items()):
            for item in list_item:
                g.add((example['{}_slip_plane_{}'.format(mat_id,i)], DISO.hasNormalVector, example['{}_slip_plane_normal_{}'.format(mat_id,i)]))
                g.add((example['{}_slip_plane_normal_{}'.format(mat_id,i)], RDF.type, DISO.SlipPlaneNormal))
                g.add((example['{}_slip_plane_normal_{}'.format(mat_id,i)], DISO.directionMillerIndice, Literal(item, datatype=XSD.string)))
        
        counter = 0
        for (i, (key, list_item)) in enumerate(slip_plane_configs.items()):
            for (j, item) in enumerate(list_item):
                g.add((crystal_structure, DISO.hasSlipSystem, example['{}_slip_system_{}'.format(mat_id,counter)]))
                g.add((example['{}_slip_system_{}'.format(mat_id,counter)], RDF.type, DISO.SlipSystem))
                g.add((example['{}_slip_system_{}'.format(mat_id,counter)], DISO.hasSlipPlaneNormal, example['{}_slip_plane_normal_{}'.format(mat_id,i)]))
                g.add((example['{}_slip_system_{}'.format(mat_id,counter)], DISO.hasSlipDirection, example['{}_slip_plane_{}_slip_direction_{}'.format(mat_id,i,j)]))
                counter +=1
    return g